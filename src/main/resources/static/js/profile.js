...
async function fetchCreditCards(userId) {
    try {
        const response = await fetch(`http://localhost:8080/api/credit-cards/user/${userId}`);
        if (!response.ok) {
...
async function deleteCreditCard(cardId, userId) {
    try {
        const response = await fetch(`http://localhost:8080/api/credit-cards/${cardId}/user/${userId}`, {
            method: "DELETE",
        });
